-- ============================================================================
-- Dados pessoais Fase 4: saúde e risco — adiciona peso, altura, patologias,
-- medicamentos, fumo e uso de moto ao cliente, e atualiza create_client_full
-- para persistir os novos campos.
--
-- Migration não destrutiva: apenas adiciona colunas com valores padrão
-- seguros, sem alterar dados existentes.
-- ============================================================================

alter table clients
  add column if not exists peso_kg           numeric(5, 2),
  add column if not exists altura_cm         integer,
  add column if not exists possui_patologia  boolean not null default false,
  add column if not exists patologias        text,
  add column if not exists usa_medicamentos  boolean not null default false,
  add column if not exists medicamentos      text,
  add column if not exists fuma              boolean not null default false,
  add column if not exists anda_moto         boolean not null default false,
  add column if not exists frequencia_moto   text;
