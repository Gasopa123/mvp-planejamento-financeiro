-- ============================================================================
-- Patrimônio Fase 1: detalhes de imóveis e automóveis.
--
-- Migration não destrutiva: mantém renda_mensal como total mensal usado nos
-- cálculos atuais e guarda subtipo, modelo e dados de financiamento dos bens.
-- ============================================================================

alter table properties
  add column if not exists subtipo text,
  add column if not exists modelo text,
  add column if not exists financiamento_termino date,
  add column if not exists parcela_financiamento numeric(14, 2);
...
begin
  ...
  insert into clients (
    ...
    salario_liquido, outras_rendas, renda_mensal, despesa_mensal_base, despesas_temporarias, despesa_mensal, patrimonio_investido, local_aplicado, tem_investimento_exterior, valor_investimento_exterior,
  ...
  values (
    ...
    coalesce((payload ->> 'despesa_mensal_base')::numeric, (payload ->> 'despesa_mensal')::numeric),
    coalesce(payload -> 'despesas_temporarias', '[]'::jsonb),
    (payload ->> 'despesa_mensal')::numeric,
    (payload ->> 'patrimonio_investido')::numeric,
    payload ->> 'local_aplicado',
    coalesce((payload ->> 'tem_investimento_exterior')::boolean, false),
    (payload ->> 'valor_investimento_exterior')::numeric,
  ...
  insert into properties (client_id, tipo, valor, financiado, adquirido_apos_casamento, subtipo, modelo, financiamento_termino, parcela_financiamento)
  select
    v_client_id,
    ...
    p.subtipo,
    p.modelo,
    p.financiamento_termino,
    p.parcela_financiamento
  ...
  select
    ...
    a.subtipo,
    a.modelo,
    a.financiamento_termino,
    a.parcela_financiamento
